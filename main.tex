...
\newpage
\thispagestyle{fancy}
\setcounter{tocdepth}{3}  % 设置目录深度：显示到 subsubsection
% 设置目录样式
\renewcommand{\cftsecfont}{\normalfont}  % section 字体
\renewcommand{\cftsubsecfont}{\normalfont}  % subsection 字体
\renewcommand{\cftsubsubsecfont}{\normalfont}  % subsubsection 字体
\renewcommand{\cftsecpagefont}{\normalfont}  % section 页码字体
\renewcommand{\cftsubsecpagefont}{\normalfont}  % subsection 页码字体
\renewcommand{\cftsubsubsecpagefont}{\normalfont}  % subsubsection 页码字体
% 设置目录标题
...
拉格朗日插值公式为：
\[f(k)=\sum_{i=0}^{n} y_i \prod_{\substack{j=0\\ j\ne i}}^{n} \frac{k - x_j}{x_i - x_j}\]
...
结论：多个下凸函数的\inlineformula{./assets/formulas_tex/formula_684.tex}仍然是下凸函数，多个上凸函数的\inlineformula{./assets/formulas_tex/formula_685.tex}仍然是上凸函数}
    ...
    \subsection{Berlekamp–Massey | Berlekamp–Massey 算法}
        \makeatletter
        \let\oldaddcontentsline\addcontentsline
        \renewcommand{\addcontentsline}[3]{%
            \ifnum\pdfstrcmp{#1}{toc}=0%
                \ifnum\pdfstrcmp{#2}{subsubsection}=0%
                    % 如果是 subsubsection，不添加到目录
                \else
        ...
        \markdownInput{berlekamp-massey.md}
