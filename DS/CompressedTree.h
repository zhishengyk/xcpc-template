...
namespace OY {
    namespace CPTREE {
        ...
        template <typename Monoid, typename SizeType = uint64_t, template <typename> typename BufferType = VectorBufferWithCollect>
        class Tree {
    ...
    template <typename Tp, Tp Identity, template <typename> typename BufferType = VectorBufferWithCollect, typename Operation, typename InitMapping, typename TreeType = CPTREE::Tree<CPTREE::BaseMonoid<Tp, Identity, Operation>, CPTREE::size_type, BufferType>>
    auto make_CompressedTree(CPTREE::size_type length, Operation op, InitMapping mapping) -> TreeType { return TreeType(length, mapping); }
    template <typename Tp, Tp Minimum = std::numeric_limits<Tp>::min(), typename SizeType = uint64_t>
    using VectorCompressedMaxTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, Minimum, CPTREE::ChoiceByCompare<Tp, std::less<Tp>>>, SizeType>;
    template <typename Tp, Tp Maximum = std::numeric_limits<Tp>::max(), typename SizeType = uint64_t>
    using VectorCompressedMinTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, Maximum, CPTREE::ChoiceByCompare<Tp, std::greater<Tp>>>, SizeType>;
    template <typename Tp, typename SizeType = uint64_t>
    using VectorCompressedGcdTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, Tp(0), CPTREE::FpTransfer<Tp, std::gcd<Tp>>>, SizeType>;
    template <typename Tp, typename SizeType = uint64_t>
    using VectorCompressedLcmTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, Tp(1), CPTREE::FpTransfer<Tp, std::lcm<Tp>>>, SizeType>;
    template <typename Tp, Tp OneMask = Tp(-1), typename SizeType = uint64_t>
    using VectorCompressedBitAndTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, OneMask, std::bit_and<Tp>>, SizeType>;
    template <typename Tp, Tp ZeroMask = 0, typename SizeType = uint64_t>
    using VectorCompressedBitOrTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, ZeroMask, std::bit_or<Tp>>, SizeType>;
    template <typename Tp, Tp ZeroMask = 0, typename SizeType = uint64_t>
    using VectorCompressedBitXorTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, ZeroMask, std::bit_xor<Tp>>, SizeType>;
    template <typename Tp, Tp Zero = Tp(), typename SizeType = uint64_t>
    using VectorCompressedSumTree = CPTREE::Tree<CPTREE::BaseMonoid<Tp, Zero, std::plus<Tp>>, SizeType>;
}
